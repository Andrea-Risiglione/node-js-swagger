...
export async function getPostComments(req, res) {
	...
	try {
		if (!postId) res.status(501).send({ error: 'Invalid Post' });

		CommentModel.find({ post: postId }, function (err, data) {
			if (err) return res.status(500).send({ err });
			if (!data)
				return res.status(501).send({
					error: "Couldn't Find the Comments for this Posts",
				});
...
export async function getComment(req, res) {
	...
	try {
		if (!commentId)
			return res.status(501).send({ error: 'Invalid Comment' });

		CommentModel.find({ _id: commentId }, function (err, comment) {
			if (err) res.status(500).send({ err });
			if (!post)
				res.status(501).send({ error: "Couldn't Find the Comment" });
